...
function getDatabaseErrorMessage(action: "load" | "create" | "update" | "delete", errorMessage: string) {
  ...
  if (normalizedMessage.includes("column") && normalizedMessage.includes("does not exist")) {
    return `Announcement ${action} failed — run supabase/add-scheduled-publish.sql in the Supabase SQL editor first. (${errorMessage})`;
  }
...
export default function AnnouncementForm() {
  ...
  async function loadAnnouncements() {
    const { data, error } = await supabase
      .from("announcements")
      .select("id,title,content,created_at,status,scheduled_for")
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const schedule = resolveSchedulePayload(scheduledForLocal);
    const payload = {
      title,
      content,
      ...
      if (error) {
        ...
        setMessage(
          schedule.status === "scheduled"
            ? `Announcement scheduled for ${formatScheduledLabel(schedule.scheduled_for)}.`
  ...
  function handleEdit(announcement: AnnouncementRecord) {
    ...
    setScheduledForLocal(
      announcement.status === "scheduled"
        ? toDatetimeLocalValue(announcement.scheduled_for)
        : "",
    );
    setMessage("");
  }
  ...
  const firstPublishedId = announcements.find((a) => a.status === "published")?.id;
  const isScheduling = Boolean(scheduledForLocal.trim());
  const submitBusyLabel = editingAnnouncementId
    ? "Updating..."
    : isScheduling
      ? "Scheduling..."
      : "Publishing...";
  const submitIdleLabel = editingAnnouncementId
    ? "Update Announcement"
    : isScheduling
    ...
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      >
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {editingAnnouncementId
                ? "Update this message for all students"
        ...
        <div className="space-y-6 p-4 sm:p-8">
          ...
          <div className="space-y-2">
            <label className="ml-1 text-sm font-semibold text-foreground" htmlFor="announcement-schedule">
            ...
            <input
              ...
              className="w-full p-4 transition-all border outline-none bg-background border-border rounded-xl focus:ring-2 focus:ring-primary focus:border-transparent"
            />
            <p className="ml-1 text-xs text-muted-foreground">
              Optional. Leave blank to publish immediately. Future times stay hidden until then.
            </p>
          </div>
          ...
          {message && (
            <div
              className={`flex items-center gap-3 p-4 rounded-xl border animate-in fade-in slide-in-from-top-2 duration-300 ${
      ...
      <div className="overflow-hidden transition-all border shadow-xl bg-card rounded-2xl border-border">
        <div className="flex flex-col gap-2 border-b border-border bg-muted/40 px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-8 sm:py-6">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Scheduled items stay private until their publish time.
            </p>
          </div>
              ...
              >
                <div className="space-y-2">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {announcement.status === "scheduled" ? (
                      <span className="inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-bold tracking-wide rounded-full bg-amber-500/15 text-amber-700 border border-amber-500/30">
                        <Clock size={10} />
                        SCHEDULED
                      </span>
                    ) : announcement.status === "draft" ? (
                      <span className="px-2 py-0.5 text-[10px] font-bold tracking-wide rounded-full bg-muted text-muted-foreground">
                        DRAFT
                      </span>
                    ) : announcement.id === firstPublishedId ? (
                      <span className="px-2 py-0.5 text-[10px] font-bold tracking-wide rounded-full bg-secondary text-secondary-foreground">
                        LATEST
                      </span>
                    ) : (
                      <span className="px-2 py-0.5 text-[10px] font-bold tracking-wide rounded-full bg-emerald-500/15 text-emerald-700">
                        PUBLISHED
                      </span>
                    )}
                  </div>
                  ...
                  {announcement.status === "scheduled" && announcement.scheduled_for ? (
                    <p className="text-xs font-medium text-amber-700">
                      Goes live {formatScheduledLabel(announcement.scheduled_for)}
                    </p>
                  ) : (
      ...
      <p className="mt-6 text-xs text-center text-muted-foreground">
        Note: Only published announcements are visible to students.
      </p>
    </div>
  );
}
