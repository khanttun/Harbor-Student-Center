...
import { Clock, Pencil, Trash2, Upload, X } from "lucide-react";
import {
...
function getDatabaseErrorMessage(action: "load" | "create" | "update" | "delete", errorMessage: string) {
  ...
  if (normalizedMessage.includes("column") && normalizedMessage.includes("does not exist")) {
    return `Event ${action} failed — run supabase/add-scheduled-publish.sql in the Supabase SQL editor first. (${errorMessage})`;
  }
...
export default function EventForm() {
  ...
  async function loadEvents() {
    const { data, error } = await supabase
      .from("events")
      .select("id,title,date,description,image_url,category,created_at,status,scheduled_for")
      ...
      if (error) {
        ...
        setMessage(
          schedule.status === "scheduled"
            ? `Event scheduled for ${formatScheduledLabel(schedule.scheduled_for)}.`
  ...
  function handleEdit(event: EventRecord) {
    ...
    setScheduledForLocal(
      event.status === "scheduled" ? toDatetimeLocalValue(event.scheduled_for) : "",
    );
    setMessage("");
  }
      ...
      <form onSubmit={handleSubmit} className="space-y-6 rounded-xl border border-border bg-card p-4 shadow-sm sm:p-6">
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium text-foreground" htmlFor="event-schedule">
            ...
            <p className="text-xs text-muted-foreground">
              Optional. Leave blank to publish immediately. Future times stay hidden from the public site until then.
            </p>
          </div>
        </div>
                ...
                <div className="space-y-2">
                  ...
                  {event.status === "scheduled" && event.scheduled_for ? (
                    <p className="text-xs font-medium text-amber-700">
                      Goes live {formatScheduledLabel(event.scheduled_for)}
                    </p>
                  ) : null}
