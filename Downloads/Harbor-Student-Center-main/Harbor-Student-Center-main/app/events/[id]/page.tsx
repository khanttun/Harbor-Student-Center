...
function isPubliclyVisible(row: { status?: string | null; scheduled_for?: string | null }) {
    if (row.status === "published" || !row.status) {
        return true;
    }
...
export default async function EventDetailPage({ params }: EventDetailPageProps) {
    ...
    const { data, error } = await supabase
        .from("events")
        .select("id,title,date,description,image_url,category,status,scheduled_for")
        .eq("id", id)
        .maybeSingle();
